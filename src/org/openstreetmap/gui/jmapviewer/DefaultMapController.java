...
MouseWheelListener {
    ...
    private void maybeShowPopup(MouseEvent e) 
    {
        if( e.isPopupTrigger() )
        {
            ...
            MapMarker[] lMarkers = map.getMapMarkersAt(lastClickedPoint);
            if( lMarkers.length > 1 )
            {
                JOptionPane.showMessageDialog(map, "Multiple markers selected. Select either a single marker, or an unmarked area of the map.", "Marker Selection Error", JOptionPane.ERROR_MESSAGE);
            }
            else if( lMarkers.length == 0 )
            {
                ...
                menuItem.addActionListener(new ActionListener() 
                {
                    public void actionPerformed(ActionEvent e) 
                    {
                        ...
                        int lResult = JOptionPane.showConfirmDialog(map, lMIP, "New Marker Data", JOptionPane.OK_CANCEL_OPTION);
            ...
            else if( lMarkers.length == 1 )
            {
                ...
                menuItem.addActionListener( new ActionListener() 
                {
                    public void actionPerformed(ActionEvent e)
                    {
                        int lResult = JOptionPane.showConfirmDialog(map, "Delete maker \"" + lSelected.getName() + "\"\nAre you sure?" , "Confirm Marker Deletion", JOptionPane.YES_NO_OPTION);
                        if( lResult == JOptionPane.YES_OPTION )
                        {
                            map.removeMapMarker(lSelected);
                            List<MapMarker> lMarkers = map.getMapMarkerList();
                            try
                            {
                                String lFile = SettingManager.getInstance().getDataFileName();
                ...
                menuItem.addActionListener( new ActionListener() 
                {
                    public void actionPerformed(ActionEvent e) 
                    {
                        ...
                        int lResult = JOptionPane.showConfirmDialog(map, lMIP, "Edit Marker Data", JOptionPane.OK_CANCEL_OPTION);
                        if( lResult == JOptionPane.OK_OPTION )
                        {    
    ...
    public void mouseClicked(MouseEvent e) 
    {
        ...
        {
            MapMarker[] lMarkers = map.getMapMarkersAt(e.getPoint());
            String lMessage = "";
            for( MapMarker marker : lMarkers )
            {
                lMessage += marker.getName() + "\n" + marker.getDescription() + "\n";
            }
            JOptionPane.showMessageDialog(map, lMessage, "Location Information", JOptionPane.INFORMATION_MESSAGE);
        }
    }
